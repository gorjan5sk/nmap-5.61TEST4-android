...
all: lua_build liblinear_build pcap_build pcre_build dnet_build nbase_build nsock_build   netutil_build
	$(MAKE) $(TARGET) $(BUILDZENMAP) $(BUILDNDIFF) $(BUILDNPING)

$(TARGET): $(top_srcdir)/liblua/liblua.a $(top_srcdir)/liblinear/liblinear.a $(LIBPCAPDIR)/libpcap.a libpcre/libpcre.a $(top_srcdir)/libdnet-stripped/src/.libs/libdnet.a $(NBASEDIR)/libnbase.a $(NSOCKDIR)/src/libnsock.a libnetutil/libnetutil.a $(OBJS)
	@echo Compiling nmap
	rm -f $@
...
lua_build: $(LIBLUADIR)/Makefile
	@echo Compiling liblua; cd $(LIBLUADIR) && $(MAKE) liblua.a CC="$(CC)" MYCFLAGS="$(CFLAGS) -DLUA_USE_POSIX"
...
clean: lua_clean liblinear_clean pcap_clean pcre_clean dnet_clean nsock_clean nbase_clean netutil_clean my_clean    
...
distclean: my_clean my_distclean lua_dist_clean liblinear_dist_clean pcap_dist_clean pcre_dist_clean dnet_dist_clean     nsock_dist_clean nbase_dist_clean netutil_dist_clean
...
install: install-nmap $(INSTALLNSE) $(INSTALLZENMAP)   $(INSTALLNDIFF) $(INSTALLNPING)
	@echo "NMAP SUCCESSFULLY INSTALLED"

uninstall: uninstall-nmap $(UNINSTALLZENMAP)   $(UNINSTALLNPING)
...
zoneTrans db2-info db2-brute html-title robots.txt xmpp)
